...
def page_col_review() -> None:
    ...
    if getattr(phase1, "reused_columns", 0):
        st.info(
            ...
            f"uploaded Past Audit Settings file — only the rest were sent to the AI."
        )
    ...
    for col in phase1.headers:
        ...
        if col in reused_cols:
            # This column's mapping came from a Past Audit Settings file — that's a human's
...
def page_rule_review() -> None:
    ...
    if getattr(phase2, "reused_rules", 0):
        st.info(
            ...
            f"uploaded Past Audit Settings file — only the rest were sent to the AI for filtering."
        )
...
def page_rule_check_review() -> None:
    ...
    if getattr(phase3, "reused_checks", 0):
        st.info(
            ...
            f"uploaded Past Audit Settings file — only the rest were generated by the AI."
        )
...
def _render_dataset_results(results, ds_name: str, show_name: bool = False) -> None:
    ...
    with dl2:
        st.download_button(
            "⬇ Download Past Audit Settings (.json)",
